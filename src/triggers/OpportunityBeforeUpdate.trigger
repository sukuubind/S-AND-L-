trigger OpportunityBeforeUpdate on Opportunity (before update) {
	...
	system.debug('***Profile not found in custom settings');
    ...
    if (!loggedInUser.Profile.Name.contains('Integration')) {
        ...
        for(Opportunity c : Trigger.new) {
            ...
            if (recordTypeName_to_recordTypeId.get('Student Opportunity') == RecTtype) {
                TriggerIds.add(c.Id);      
            }           
        }
        for (integer record = 0; record < Trigger.new.size(); record++) { 
            ...
            if(TriggerIds.contains(newRecord.Id)){
                ...
                if (newRecord.Student__c != oldRecord.Student__c ) {
                    updateOppsContactChangedOLD.add(oldRecord);
                    updateOppsContactChangedNEW.add(newRecord);
                }
                
                if (newRecord.Program__c != oldRecord.Program__c ) {
                ...
                if(newRecord.webschedulerstatus__c == 'New'){ 
                 if((oldRecord.webschedulerstatus__c == 'Scheduled') || (oldRecord.webschedulerstatus__c == 'Rescheduled')){
        ...
        if (updateOppsSDChangeOLD.size() > 0 && updateOppsSDChangeNEW.size() > 0) {
            ...
            Opp_StartDateChanged.findAcademicStartDateRecords(updateOppsSDChangeNEW);
        }
        ...
        if (updateOppsSDChangedToNullNEW.size() > 0) {
            ...
            Opp_StartDateChanged.startDateChangedToNull(updateOppsSDChangedToNullNEW);
        }
        ...
        if (updateOppsContactChangedOLD.size() > 0 && updateOppsContactChangedNEW.size() > 0) {
            System.debug('--- OpportunityBeforeUpdate - Referred By changed to something or changed to NULL------');
            Opp_StudentChanged.studentChanged(updateOppsContactChangedOLD, updateOppsContactChangedNEW);
        }
        ...
        if (updateOppsProgramChangedOLD.size() > 0 && updateOppsProgramChangedNEW.size() > 0) {
            ...
            Opp_ProgramChanged.programChangedToNull(updateOppsProgramChangedOLD, updateOppsProgramChangedNEW);
